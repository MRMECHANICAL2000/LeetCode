"""
Key : Know how to use an global variable, if not you will need to put its name in all the function and 
	  your code wont look clean **IMPORTANT** Must Must. look into our note ones and look the important
...
class Solution:
    ...
    def solveNQueens(self, n: int) -> List[List[str]]:
        ...
        output=[]            #To output in required format
        for i in Result:
            temp=[]
            for j in i:
                temp.append(str( "."*j + "Q" + "."*(n-j-1) ))
            output.append(temp)
        return(output)
